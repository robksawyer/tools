...
class GoogleMapV3Helper extends AppHelper {
	...
	protected $_defaultOptions = array(
		...
		'map'=>array(
			'api' => "AIzaSyD09vDtMx9urWkcrWqB10eZs-MhJJYfaBI", //REPLACE WITH YOUR API KEY
...
/** Google Maps JS **/
	...
	public function setControls($options = array()) {
		...
		if (isset($options['disableDoubleClickZoom'])) {
			$this->_currentOptions['map']['disableDoubleClickZoom'] = $options['disableDoubleClickZoom'];
		}
		if (isset($options['draggable'])) {
			$this->_currentOptions['map']['draggable'] = $options['draggable'];
		}
		if (isset($options['scrollwheel'])) {
			$this->_currentOptions['map']['scrollwheel'] = $options['scrollwheel'];
		}
		if (isset($options['panControl'])) {
			$this->_currentOptions['map']['panControl'] = $options['panControl'];
		}
		/*
	...
	*/
	...
	if (navigator.geolocation) {
		...
		navigator.geolocation.getCurrentPosition(function(position) {
			...
			saveLocationData(position.coords.latitude, position.coords.longitude);
		}, function() {
		...
		geo.getCurrentPosition(function(position) {
			...
			saveLocationData(position.latitude, position.longitude);
		}, function() {
			handleNoGeoLocation(browserSupportFlag);
		});
	...
	protected function _geolocationSaveCallback() {
		if (($js = $this->_currentOptions['callbacks']['save_location_callback']) === false) {
			return '';
		}
		if ($js === null) {
			return '';
...
/** Google Maps Link **/
	...
	/**
	 * This method handles getting the user's location
	 */
...
*/
	 ...
	 * @return void
	 */
	public function setCluster($options = array()) {
	...
	public function addClusterMarker($marker, $options = array()) {
